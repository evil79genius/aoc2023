...
namespace _11B
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            cntEmpty = 0;
            var columnExpansion = Enumerable.Range(0, columns.Max() + 1).Select(c => { if (!columns.Contains(c)) cntEmpty += 999999; return cntEmpty; }).ToArray();
            var rows = stars.Select(s => s.y).Distinct();
            cntEmpty = 0;
            var rowExpansion = Enumerable.Range(0, rows.Max() + 1).Select(r => { if (!rows.Contains(r)) cntEmpty += 999999; return cntEmpty; }).ToArray();
